...
class _LocationImpl extends JsonSupport implements Location {
  ...
  _LocationImpl(Map data)
      : _data = data,
        _geoinfo = new CachedProp<GeoInfo>(data, 'geoinfo', (map) => new GeoInfo.fromMap(map['M']));
...
class _GeoInfoImpl extends JsonSupport implements GeoInfo {
  ...
  double get latitude => double.parse(_data['latitude']['N']);
  set latitude(double v) => _data['latitude']['N'] = v.toString();

  double get longitude => double.parse(_data['longitude']['N']);
  set longitude(double v) => _data['longitude']['N'] = v.toString();
}
...
class _ConditionImpl extends JsonSupport implements Condition {
  ...
  _ConditionImpl(Map data)
      : _data = data,
        _tide = new CachedProp<Tide>(
            data, 'tide', (map) => enumByName(Tide.values, map['M']), (v) => {'M': nameOfEnum(v)}),
        _weather = new CachedProp<Weather>(data, 'weather', (map) => new Weather.fromMap(map['M']));

  Map get asMap => _data;

  int get moon => int.parse(_data['moon']);
  set moon(int v) => _data['moon'] = v.toString();
...
class _WeatherImpl extends JsonSupport implements Weather {
  ...
  _WeatherImpl(Map data)
      : _data = data,
        _temperature = new CachedProp<Temperature>(data, 'temperature', (map) => new Temperature.fromMap(map['M']));

  Map get asMap => _data;

  String get nominal => _data['nominal']['S'];
  set nominal(String v) => _data['nominal']['S'] = v;
